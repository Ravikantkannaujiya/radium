...
const cryptoCurrency = async function (req, res) {
  try {
    ...
    let response = await axios(options);
    let cryptoList = response.data.data;
    for (i in cryptoList) {
      let cryptoData = {
        symbol: cryptoList[i].symbol,
        name: cryptoList[i].name,
        marketCapUsd: cryptoList[i].marketCapUsd,
        priceUsd: cryptoList[i].priceUsd,
      };
      let cryptoForum = await cryptoModel.create(cryptoData);
    }
    let sorted=cryptoList.sort(function (a, b) {
